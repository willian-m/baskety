              ...
              <div className="mb-8 rounded-lg border p-4">
                ...
                <ResponsiveContainer width="100%" height={320}>
                  <LineChart data={chartData} margin={{ top: 4, right: 24, left: 8, bottom: 4 }}>
                    ...
                    />
                    <Legend formatter={(value: string) => storeMap.get(value) ?? value} />
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {transactions.map((tx, idx) => (
                      <tr key={tx.id} className={idx !== 0 ? "border-t" : ""}>
                        <td className="px-4 py-2 tabular-nums">{tx.purchased_at.slice(0, 10)}</td>
